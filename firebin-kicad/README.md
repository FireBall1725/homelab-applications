...
| Host | Description |
|------|-------------|
| `firebin-kicad.k8s.firekatt.ca` | FireBin KiCad server (internal LAN only) |
...
|--------|------|---------------|-------|
| `firebin-kicad-data` | 1Gi | `longhorn` | Holds the inbound MCP bearer token (RWO) |
...
|--------|----------|--------|
| `firebin-kicad-access-token` | `FIREBIN_ACCESS_TOKEN`, the outbound `fbin_pat_` PAT | SealedSecret (`templates/sealed-secret-access-token.yaml`) |
| `firebin-kicad-token` | `FIREBIN_MCP_TOKEN`, the inbound bearer clients present on `/mcp` | SealedSecret (`templates/sealed-secret-mcp-token.yaml`) |
